...
import { apiClient, type ProjectImage } from "@/lib/api";
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const presigned = await apiClient.getImageUploadUrl(
          projectId,
          file.name,
          file.type,
          file.size
        ...
        const completedImage = await apiClient.completeImageUpload(
          projectId,
          presigned.image_id,
          dimensions
        );
